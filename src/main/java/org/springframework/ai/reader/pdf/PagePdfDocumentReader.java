...
public class PagePdfDocumentReader implements DocumentReader {
		...
		try {
			...
			for (PDPage page : this.document.getDocumentCatalog().getPages()) {
				...
				if (StringUtils.hasText(pageText)) {
					...
					if(!isPageNumberExist(pageText)) {
						pageText = pageText.concat("             "+String.valueOf(counter)+"           ");
					}
	...
	private boolean isPageNumberExist(String pageText) {
	
		 String[] arrOfstr = pageText.split("\n");
		 
		 String lastLine =  arrOfstr[arrOfstr.length-1];
